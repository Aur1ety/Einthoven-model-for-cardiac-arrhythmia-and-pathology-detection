...
// Headings are numbered 4.1 to 4.5 by a CSS counter (.sec-numbered / .sec-h): display only, so their names do not change.
function Section({ title, children }) {
  return (
    <section className="space-y-3">
      <h2 className="t-title sec-h">{title}</h2>
      {children}
    </section>
  )
}

// File paths break after their slashes (<wbr>), or anywhere if they still do not fit a 320 px screen.
const PATH = '[overflow-wrap:anywhere]'
...
export default function MethodView({ site }) {
  return (
    <div className="measure">
      ...
      <div className="sec-numbered space-y-8" style={{ '--sec-prefix': "'4.'" }}>
        <Section title="Data">
          <p className="t-body">
        ...
        <Section title="What this site shows, and how it is kept honest">
          <ul className="list-dash t-body">
            ...
            <li>
              When the site’s data is exported (<code className={PATH}>web/<wbr />scripts/<wbr />export_site_data.py</code>, run before each deploy, not by the
            ...
            <li>
              The 3D view applies the released Kors (1990) coefficients (the file <code className={PATH}>data/<wbr />processed/<wbr />vcg_kors_coefficients.json</code>)
        ...
        <Section title="Sources and licences">
          <ol className="refs t-body" role="list">
            <li>
